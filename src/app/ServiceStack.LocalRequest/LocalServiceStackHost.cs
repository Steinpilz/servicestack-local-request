...
{
    public class LocalServiceStackHost : IAppHost, IHasContainer, IRunnableAppHost
    {
        public const string RetryRequestMarker = "RetryCurrentRequest";

        private Action<EndpointHostConfig> _config;

        protected LocalServiceStackHost()
        {
            EndpointHostConfig.SkipPathValidation = true;
        }

        public LocalServiceStackHost(string serviceName, params Assembly[] assembliesWithServices)
            : this()
        {
            EndpointHost.ConfigureHost(this, serviceName, CreateServiceManager(assembliesWithServices));
            EndpointHost.VirtualPathProvider = new FileSystemVirtualPathProvider(this,
        ...
        public bool Handle(IHttpRequest httpReq, IHttpResponse httpRes)
        {
            ...
            {
                var error = string.Format("Error this.ProcessRequest(context): [{0}]: {1}", ex.GetType().Name,
                    ex.Message);

                Config.LogFactory.GetLogger(GetType()).Error(error);
                ...
                catch (Exception errorEx)
                {
                    error =
                        string.Format(
                            "Error this.ProcessRequest(context)(Exception while writing error to the response): [{0}]: {1}",
        ...
        protected virtual bool ProcessRequest(IHttpRequest httpReq, IHttpResponse httpRes)
        {
            ...
            if (serviceStackHandler == null)
                throw new NotImplementedException("Cannot execute handler: " + handler + " at PathInfo: " +
                                                  httpReq.PathInfo);
...
/*ignore*/
        ...
        public virtual void LoadPlugin(params IPlugin[] plugins)
        {
            foreach (var plugin in plugins)
            {
                ...
                {
                    Config.LogFactory.GetLogger(GetType()).Warn("Error loading plugin " + plugin.GetType().Name, ex);
                }
            }
        }
